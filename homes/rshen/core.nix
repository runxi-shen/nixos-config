# Portable shell/editor/VCS core. Must stay username-agnostic and free of any
# absolute /Users or /home path -- everything derives from
# `config.home.homeDirectory`, which the consumer supplies.
{ config, pkgs, lib, ... }:
...
in
{
  options.rshen = {
    ...
    gitUserEmail = lib.mkOption {
      type = lib.types.str;
      default = "shenrunxi@gmail.com";
      description = ''
        Value for git's user.email. Deliberately an option rather than a
        constant: the lab servers commit under the Broad address, so neusis
        overrides this per host while the Macs keep the default.
      '';
    };
  };

  config = {
    # Defaults only. Every real consumer sets these itself -- nix-darwin from
    # `home-manager.users.<name>`, neusis from its own account definition -- and
    # a plain definition always outranks mkDefault, so they cannot collide at
    # eval. `rshen` is the standard username on every Linux machine in the fleet.
    home = {
      username = lib.mkDefault "rshen";
    ...
    programs = {
      ...
      ssh = {
        ...
        # Was a pair of absolute per-platform branches keyed on isLinux /
        # isDarwin. homeDirectory already encodes both, for any username.
